...
//-----------------------------------------
{
    int iwidth, iheight;
    GLubyte* image = NULL;
    char filename0[] = "texture/Metalcolor.ppm";
    image = glmReadPPM(filename0, &iwidth, &iheight);
...
//----------------------------------------
{
    ...
    locDeformation = glGetUniformLocation(programID, "deformation");
    locshadtype = glGetUniformLocation(programID, "shadtype");
    locthetamx = glGetUniformLocation(programID, "thetamx");
    loctmin = glGetUniformLocation(programID, "tmin");
    loctmax = glGetUniformLocation(programID, "tmax");
...
//-------------------------------------
{
    ...
    glUniform1f(locDeformation, deformation);
    glUniform1f(locshadtype, shadtype);
    glUniform1f(locthetamx, thetamx);
    glUniform1f(loctmin, tmin);
    glUniform1f(loctmax, tmax);
...
void clavier(unsigned char touche, int x, int y)
{
    switch (touche)
    {
    case 'm': /* sahder de toon ou gooch */
        if (shadtype == 1.0)
            shadtype = 0.0;
        else
            shadtype = 1.0;
